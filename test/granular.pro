...
test(granular_create, [nondet, cleanup(granular_unload(G))]) :-
...
test(granular_get_defaults, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_density, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_position, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_position_clamped, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_size, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_pitch, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_envelope, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_envelope_clamped, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_pan, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_pan_clamped, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_regularity, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_reverse_probability, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_recording, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
    granular_set(G, [recording=true]),
    granular_get(G, Params),
    memberchk(recording=true, Params).

test(granular_set_normalize_false, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
    granular_set(G, [normalize=false]),
    granular_get(G, Params),
    memberchk(normalize=false, Params).

test(granular_set_normalize_true, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
    granular_set(G, [normalize=false]),
...
test(granular_set_multiple, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_set_spray_params, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
...
test(granular_trigger, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
    granular_trigger(G).
...
test(granular_unload, [nondet]) :-
    granular_create(2.0, G),
    granular_unload(G).

test(granular_unload_invalid_handle, [error(existence_error(granular_delay, _))]) :-
    granular_unload(granular(999)).

% Connect tests with sound source

test(granular_connect_sound, [nondet, cleanup((granular_unload(G), sound_unload(S)))]) :-
    granular_create(2.0, G),
...
test(granular_connect_voice, [nondet, cleanup((granular_unload(G), synth_voice_unload(V)))]) :-
    granular_create(2.0, G),
...
test(granular_connect_invalid_source, [error(existence_error(source, _)), cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
    granular_connect(G, sound(999)).

% Mode tests

test(granular_set_mode_major, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
    granular_set_mode(G, [0.0, 2.0, 4.0, 5.0, 7.0, 9.0, 11.0], 0, 6).

test(granular_set_mode_pentatonic, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
    granular_set_mode(G, [0.0, 3.0, 5.0, 7.0, 10.0], 0, 10).

test(granular_set_mode_disable, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
    granular_set_mode(G, [0.0, 2.0, 4.0], 0, 2),
    granular_set_mode(G, [], 0, 0).

test(granular_set_mode_with_deviation_down, [nondet, cleanup(granular_unload(G))]) :-
    granular_create(2.0, G),
